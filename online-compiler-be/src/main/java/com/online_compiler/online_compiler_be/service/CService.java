...
@Service
public class CService implements LanguageExecutionService {

	/**
	 * Reuses the same GCC image as C++ (it ships both gcc and g++), so no
	 * extra image needs to be pulled/maintained for this language.
	 */
	private static final String IMAGE = "gcc:13.2.0";
	private static final Set<String> SUPPORTED_STANDARDS = Set.of("90", "99", "11", "17", "23");
	...
	public String run(String code, String version) throws Exception {
		...
		try (TempWorkspace workspace = TempWorkspace.create("c-code")) {
			workspace.writeSourceFile("main.c", code);
			...
			DockerCommandRunner.run(List.of(
					...
					IMAGE,
					"gcc", "-std=c" + resolveStandard(version), "-O2", "-o", "/app/main", "/app/main.c"
